-- lua/kape/theme/groups/init.lua
-- Groups registry with auto-discovery.
-- Discovers all *.lua files in this directory (except init.lua itself)
-- and treats each as a Kape.GroupSpec.

---@class Kape.Theme.Groups
local M = {}

--- Cache for loaded specs
---@type Kape.GroupSpec[]|nil
local specs_cache = nil

--- Discover and load all group specs from this directory
---@return Kape.GroupSpec[]
function M.load_specs()
  ...
  for _, file in ipairs(vim.fn.glob(groups_dir .. "/*.lua", false, true)) do
    ...
    if name ~= "init" then
      local module_path = "kape.theme.groups." .. name
